...
func (s *ConcurrentArray[T]) Merge(list *List[T]) {
    s.mu.Lock()
    defer s.mu.Unlock()
...
func (s *ConcurrentArray[T]) Filter(predicate func(T) bool) *List[T] {
    s.mu.RLock()
    defer s.mu.RUnlock()
...
func MapConcurrentArray[T comparable, V comparable](s *ConcurrentArray[T], mapper func(T) V) *List[V] {
    s.mu.RLock()
    defer s.mu.RUnlock()
