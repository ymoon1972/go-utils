...
func TestNewLinkedList_Add(t *testing.T) {
...
func TestNewLinkedList_Insert(t *testing.T) {
	list := NewLinkedList[int]()
	require.Equal(t, 0, list.Size(), "LinkedList size is not equal")
	require.Equal(t, true, list.IsEmpty(), "LinkedList is not empty")
	...
	validateTail(t, list, 1)
	validateAt(t, list, 0, 0)
	validateAt(t, list, 1, 1)
...
func TestNewLinkedList_Clone(t *testing.T) {
	list := NewLinkedList[int]()
	list.AddAll([]int{1, 2, 3})
...
func TestNewLinkedList_Merge(t *testing.T) {
	list := NewLinkedList[int]()
	list.AddAll([]int{1, 2, 3})
...
func TestNewLinkedList_Reverse(t *testing.T) {
	list := NewLinkedList[int]()
	list.AddAll([]int{1, 2, 3})
	list.Reverse()
...
func TestNewLinkedList_Sort(t *testing.T) {
	list := NewLinkedList[int]()
...
func TestNewLinkedList_Map(t *testing.T) {
	list := NewLinkedList[int]()
	list.AddAll([]int{1, 2, 3, 4, 5, 6})

	mapped := Map(list.Iterator(), func(item int) string { return strconv.Itoa(item) })
	require.ElementsMatch(t, []string{"1", "2", "3", "4", "5", "6"}, mapped.Values(), "LinkedList map is failed")
}

func TestNewLinkedList_Reduce(t *testing.T) {
	list := NewLinkedList[int]()
	list.AddAll([]int{1, 2, 3, 4, 5, 6})

	aggregated := Reduce(
		Map(list.Iterator(), func(item int) string { return strconv.Itoa(item) }).Iterator(),
		"",
		func(acc string, item string) string {
			if len(acc) == 0 {
				return item
			}
			return acc + "-" + item
		})
	require.Equal(t, "1-2-3-4-5-6", aggregated, "LinkedList reduce is failed")
}

func TestNewLinkedList_Filter(t *testing.T) {
	list := NewLinkedList[int]()
	list.AddAll([]int{1, 2, 3, 4, 5, 6})

	even := Filter(list.Iterator(), func(value int) bool { return value%2 == 0 })
...
func validateAt(t *testing.T, list *LinkedList[int], index int, expectedValue int) {
	value, err := list.GetAt(index)
	require.Nil(t, err, "LinkedList getTail is failed")
	require.Equal(t, expectedValue, value, "LinkedList tail item is not matched")
}

func validateDLinkedListRemoveHead(t *testing.T, list *LinkedList[int], expectedValue int) {
...
func validateDLinkedListRemoveTail(t *testing.T, list *LinkedList[int], expectedValue int) {
...
func validateDLinkedListRemoveAt(t *testing.T, list *LinkedList[int], index, expectedValue int) {
